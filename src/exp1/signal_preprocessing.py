...
class SignalPreprocessor:
    ...
    def export_to_csv(self, processed_data, labels_df, output_path):
        """
        Export processed data to CSV format for analysis
        
        Args:
            processed_data: Dictionary from process_all_signals()
            labels_df: DataFrame with damage labels (from Arreglo_3_actual_clean.csv)
            output_path: Path for output CSV file
        """
        print("Exporting processed dataset to CSV...")
        ...
        for specimen, sensors in processed_data.items():
            ...
            # Process each sensor (S1, S2)
            for sensor, data in sensors.items():
                features = data['features']  # Shape: (n_freqs, 3 components)
                metadata = data['metadata']
                
                # Flatten feature matrix: cada fila = una observación
                # Para cada frequency, tenemos 3 componentes (N-S, E-W, U-D)
                for freq_idx in range(features.shape[0]):
                    row = {
                        'specimen': specimen,
                        'sensor': sensor,
                        'frequency_index': freq_idx,
                        'frequency_hz': metadata['selected_frequencies'][freq_idx],
        ...
        print(f"  Specimens: {df['specimen'].nunique()}")
        print(f"  Damage levels: {dict(df['damage_level'].value_counts())}")
        print(f"  Frequency points per specimen/sensor: {df.groupby(['specimen', 'sensor']).size().iloc[0]}")
